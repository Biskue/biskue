...
import RestaurantCard from './RestaurantCard'
 ...
 class StepThree extends Component {
   ...
   componentWillMount(){
     ...
     axios.get(`/poll/search?latitude=${latitude}&longitude=${longitude}&radius=${radius}&open_at=${date}&categories=${categories}&price=${priceRange}`)
  ...
  render() {
    const restaurantsList = this.state.restaurants.map((restaurant, index)=>{
      return(
        <RestaurantCard key ={index} addRestaurant={this.addRestaurant} currentRes= {restaurant}/>
      )
    })
    const doneButton = this.state.selected.length > 0 ?( <Link to='/wizard/step-4'><button>Done</button></Link>): null
    return (
      <div>
