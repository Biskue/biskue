...
class RestaurantFinder extends Component {
    ...
    searchForRestaurant(){
        axios.get(`/poll/search?latitude=${this.props.latitude}&longitude=${this.props.longitude}&term=${this.state.search}`)
        .then((results) => this.setState({ restaurants: results.data.businesses }));
    }
  render() {
      const searchResults = this.state.restaurants.length > 0 ? this.state.restaurants.map((restaurant, index) => {
      return <RestaurantCard key={index} addRestaurant={this.props.addRestaurant} currentRes={restaurant} />}) : null
    return (
      <div>
